...
def convert_bbq_to_sceneverse(bbq_node):
    """
    Takes a single object node from BBQ's list and converts it
    to a SceneVerse ObjNode.
    """
    # Map the fields from the BBQ format to the ObjNode format
    sceneverse_node = ObjNode(
        id=bbq_node['id'],
        label=bbq_node['description'], # Or a cleaned-up version of the description
        position=np.array(bbq_node['bbox_center']),
        size=np.array(bbq_node['bbox_extent'])
    )
    return sceneverse_node
